...
test('provider account-session source separates remote reads from proof reconstruction', async () => {
  ...
  for (const code of [
    ...
    'RECOVERY_ACCOUNT_SESSION_PROOF_PROJECTION_MISMATCH',
  ]) {
...
test('recovery closes an empty fixture set without opening the 42-step checkpoint', async () => {
  ...
  assert.match(controlStore,
    /\['planned','created'\]\.includes\(primaryExecutions\[0\]\.state\)/u);
  assert.match(providerStore,
    /\['planned','created'\]\.includes\(proof\.primaryExecutionIntent\.state\)/u);
});

...
export function readQualifiedTestCloudBrowserArtifactMember(args) {
  try {
    if (!exact(args, ARG.read) || !Number.isSafeInteger(args.policyOrdinal) || args.policyOrdinal < 0 || args.policyOrdinal >= 25 || !Number.isSafeInteger(args.occurrenceIndex) || args.occurrenceIndex < 0) return block();

...
import { sha256Bytes } from '../../core/canonical-json.mjs';
import {
  createHostedSiteBuildIdentity,
  validateHostedSiteBuildIdentity,
} from '../../core/hosted-site-build-identity.mjs';
...
SOURCE_PATHS.set('verification-runner-py', 'verification/appwrite-runner');
  ...
  try {
    ...
    for (let index = 0; index < allArtifacts.length; index += 1) {
      ...
      if (
        ...
        || entry.sourcePath !== (artifact.kind === 'site' ? 'apps/web' : SOURCE_PATHS.get(artifact.logicalTarget))

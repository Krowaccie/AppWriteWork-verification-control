...
const CLEANED_BATCH_CONTEXTS = new WeakMap();
...
function validDescriptor(d,context=null){
  ...
  if(d.resourceType!=='primary-execution') {
    const order = { 'primary-project': 10, 'primary-graph': 20, 'primary-share': 30 };
    // Session creation requires the separate qualified session protocol; this
    // generic descriptor seam cannot mint its authority from caller data.
    if (!Object.hasOwn(order, d.resourceType) || !isAuthenticTestEnvironmentContext(context)) return false;
    const resourceId = `vr-${hash(`${context.environmentDigest}|${context.runId}|${d.resourceType}`).slice(0,32)}`;
    const operationKey = sha256Bytes(encoder.encode(`${context.runId}|sharing-permissions|{}`));
    const ownerMarker = `verification-owner.v1:${canonicalDigest({
      schemaVersion: 'verification-owner-marker.v1', environmentDigest: context.environmentDigest,
      operationKey, runId: context.runId, resourceType: d.resourceType, resourceId,
    })}`;
    return d.resourceId === resourceId && d.ownerMarker === ownerMarker
      && d.dependencyOrder === order[d.resourceType] && d.lifecycleClass === 'fixture'
      && d.retentionExpiresAt === null && d.providerResourceIds.length === 1
      && d.providerResourceIds[0] === resourceId;
  }
...
export async function cleanupRun({context,store,provider,lease,capability,intents,clock}) {
  ...
  try {
    ...
    for (const intent of result) {
      if (intent?.runId !== context.runId || intent.environmentDigest !== context.environmentDigest
          || intent.resourceType === 'account-session-set'
          || !TYPES.has(intent.resourceType) || identities.has(intent.intentId)
          || typeof store?.getIntentProjection !== 'function') return blocked('CLEANUP_AMBIGUOUS');
      if (intent.resourceType !== 'primary-execution'
          && !validDescriptor(descriptorFrom(intent), context)) return blocked('CLEANUP_AMBIGUOUS');
      identities.add(intent.intentId);
      const recorded = await store.getIntentProjection(intent.intentId);
      if (recorded === null || canonicalJson(recorded) !== canonicalJson(intent)) {
    ...
    const value = freeze({intents:result,lease:currentLease,capability:currentCap});
    if (result.length > 0) CLEANED_BATCH_CONTEXTS.set(result, context);
    return pass(value);
...
export async function verifyRunAbsent({context,provider,intents}) {
  try {
    if (!isAuthenticTestEnvironmentContext(context) || !Array.isArray(intents)
        || CLEANED_BATCH_CONTEXTS.get(intents) !== context
        || typeof provider?.readExact !== 'function') return blocked('CLEANUP_AMBIGUOUS');
    for (const intent of intents) {
      if (intent.lifecycleClass === 'provider-retained-observation') continue;
      if (intent.state !== 'absent') return blocked('CLEANUP_AMBIGUOUS');
      if ((await provider.readExact(intent))?.status !== 404) return blocked('CLEANUP_AMBIGUOUS');
    }
    return pass(freeze({absenceProven:true}));
  } catch { return blocked('CLEANUP_AMBIGUOUS'); }
}

...
const scriptUrl = (name) => new URL(`${name.startsWith('test-cloud-') ? 'verification/adapters/test-cloud' : 'verification/core'}/${name}`, rootUrl);

const inventoryUrl = new URL('verification/fixtures/environments/test-cloud.inventory.v1.json', rootUrl);
  ...
  if (syntheticControlModulePromise === undefined) {
    syntheticControlModulePromise = (async () => {
      ...
      for (const [from,to] of [
        ["'../../core/canonical-json.mjs'", JSON.stringify(scriptUrl('canonical-json.mjs').href)],
        ...
        ["import inventory from '../../fixtures/environments/test-cloud.inventory.v1.json' with { type: 'json' };",'const inventory=Object.freeze({control:Object.freeze({primaryExecutionRetentionMaxSeconds:86400})});'],
      ]) {
...
async function importInstrumentedPreflight() {
  ...
  const replacements = new Map([
    ['../../fixtures/environments/test-cloud.inventory.v1.json',
      new URL('verification/fixtures/environments/test-cloud.inventory.v1.json', rootUrl).href],
    ['../../core/canonical-json.mjs', scriptUrl('canonical-json.mjs').href],
    ['../../core/artifact-manifest.mjs', scriptUrl('artifact-manifest.mjs').href],

...
function validProductFunctionRecord(value) {
  return validFunctionRecord(value)
    && value.sourcePath === `services/appwrite-functions/${value.logicalId}`;
}

function validRunnerFunctionRecord(value) {
  return validFunctionRecord(value)
    && value.logicalId === 'verification-runner-py'
    && value.sourcePath === 'verification/appwrite-runner';
}
...
function validInventory(inventory) {
  ...
  if (
    ...
    || !inventory.productFunctions.every(validProductFunctionRecord)
    ...
    || !validRunnerFunctionRecord(inventory.testOnlyFunctions[0])
  ) return false;

...
import inventory from '../../fixtures/environments/test-cloud.inventory.v1.json' with {
  type: 'json',
};
...
const PREFLIGHT_DIGEST_KEYS = Object.freeze([
  'environmentDigest', 'providerContractDigest', 'identityBindingsDigest',
  'providerSetupReadbackDigest', 'runnerVariableReadbackDigest',
]);
  ...
  try {
    ...
    for (let index = 0; index < manifest.artifacts.length; index += 1) {
      ...
      const expectedSourcePath = entry.kind === 'site'
        ? 'apps/web'
        : entry.logicalTarget === 'verification-runner-py'
          ? 'verification/appwrite-runner'
          : `services/appwrite-functions/${entry.logicalTarget}`;
      if (
        artifact === undefined
    ...
    && (
      value.value.capability === null
        ...
        : value.value.capability !== undefined
          && exactDataObject(value.value.lease, E2E_LEASE_KEYS)
          && exactDataObject(failureLease, E2E_LEASE_KEYS)
          && E2E_STABLE_SUCCESSOR_LEASE_KEYS.every((key) => value.value.lease[key] === failureLease[key])
    )
  );
...
async function invoke(method, request, validateValue = null) {
  try {
    ...
    if (outcome.status === 'PASS') {
      if (validateValue !== null && !validateValue(outcome.value)) {
        return result('BLOCKED', null, 'TEST_CLOUD_PREFLIGHT_BLOCKED');
      }
      return result('PASS', outcome.value);
    }
    return result(
      outcome.status,
      null,
...
async function invokeAtomicCleanup(method, request) {
  try {
    ...
    return result('PASS', { closed: true, lease: outcome.value.lease,
      absenceDigest: digestBytes(Buffer.from(canonicalJson(outcome.value))) });
  } catch {
    return result('BLOCKED', null, 'CLEANUP_DEBT');
  }
}
...
export async function runTestCloudLane(args) {
  ...
  args = deepFreeze({
    controller: args.controller, selection: { ...args.selection }, artifactSet,
    clients: { ...args.clients }, clock: { ...args.clock }, evidenceWriter: { ...args.evidenceWriter },
  });
  ...
  const preflight = await invoke(args.clients.preflight, common, (value) => (
    exactDataObject(value, ['handoff'])
    && exactDataObject(value.handoff, [...PREFLIGHT_DIGEST_KEYS,
      'observedLeaseVersion', 'observedLedgerDigest', 'observedAtEpochSeconds'])
  ));
  primaryFailure = stageFailure(preflight, 'TEST_CLOUD_PREFLIGHT_BLOCKED', true);
  if (primaryFailure !== null) return primaryFailure;
  const preflightHandoff = Object.getOwnPropertyDescriptor(preflight.value ?? {}, 'handoff')?.value;
  const environmentBinding = {};
  for (const key of PREFLIGHT_DIGEST_KEYS) {
    const value = Object.getOwnPropertyDescriptor(preflightHandoff ?? {}, key)?.value;
    if (typeof value !== 'string' || !DIGEST.test(value)) {
      return result('BLOCKED', null, 'TEST_CLOUD_PREFLIGHT_BLOCKED');
    }
    environmentBinding[key] = value;
  }
  if (environmentBinding.environmentDigest !== digestBytes(Buffer.from(canonicalJson(inventory)))
      || environmentBinding.providerContractDigest !== inventory.providerContractDigest) {
    return result('BLOCKED', null, 'TEST_CLOUD_PREFLIGHT_BLOCKED');
  }
  ...
  if (
    ...
    || !exactDataObject(acquired.value.lease, E2E_LEASE_KEYS)
    || acquired.value.lease.environmentDigest !== environmentBinding.environmentDigest
    || acquired.value.lease.ownerWorkflowRunId !== args.selection.workflowRunId
    || acquired.value.lease.ownerRunId !== `verify-${args.selection.sourceRevision.slice(0, 12)}-${args.selection.workflowRunId}-${args.selection.workflowRunAttempt}`
    || acquired.value.lease.leaseRowId !== inventory.control.leaseRowId
    || acquired.value.lease.state !== 'active' || acquired.value.lease.cleanupDebt !== false
  ...
  try {
    ...
    if (primaryFailure === null) {
      ...
      if (primaryFailure === null) {
        if (
          ...
          || !exactDataObject(e2e.value.lease, E2E_LEASE_KEYS)
          || E2E_STABLE_SUCCESSOR_LEASE_KEYS.some((key) => e2e.value.lease[key] !== leaseState.lease[key])
    ...
    if (cleaned.status !== 'PASS') {
      ...
      cleanupAbsenceDigest = cleaned.value.absenceDigest;
    }
  }
  ...
  const binding = deepFreeze({
    ...
    sourceTreeDigest: artifactSet.artifactManifest.sourceTreeDigest,
    ...
    controller: {
      controllerRepository: args.controller.controllerRepository,
      sourceRepositoryRevision: args.controller.sourceRepositoryRevision,
      controllerBundleSha: args.controller.controllerBundleSha,
      controllerBundleDigest: args.controller.controllerBundleDigest,
      controllerArtifactId: args.controller.controllerArtifactId,
    },
  ...
  const evidence = deepFreeze({
    binding,
    bindingDigest: digestBytes(Buffer.from(canonicalJson(binding))),
    startedAt,
    completedAt,

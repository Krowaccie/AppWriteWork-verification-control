...
import { produceControllerRunnerQualification } from '../../core/controller-trust-materials.mjs';
...
function proposal() {
  return {
    ...
    seedSourceSets: {
      ...
      path: 'verification/controller/controller-seed-source-sets.v1.json',
      schemaPath: 'verification/schemas/controller-seed-source-sets.v1.schema.json',
    },
...
async function importPublisherVariant({ blockManifest = false, blockWrite = false }) {
  ...
  if (blockManifest) {
    const materializerImport =
      "import { materializeControllerBundleProposal } from '../../core/controller-bundle.mjs';";
...
async function createCommittedCliFixture(t, prefix) {
  ...
  const fixtureFiles = new Map([
    ['verification/controller/controller-bundle.proposal.json', `${JSON.stringify(fixtureProposal)}\n`],
...
test('publisher materializes direct canonical members from exact controller SHA and same-run trust inputs', async () => {
  ...
  assert.ok(paths.includes('verification/controller/controller-bundle.manifest.json'));
...
test('publication writer emits exactly the direct member tree and no nested archive', async (t) => {
  ...
  const manifest = JSON.parse(await readFile(path.join(outputRoot, 'verification', 'controller', 'controller-bundle.manifest.json'), 'utf8'));
  assert.equal(manifest.controllerRevision, CONTROLLER_SHA);
  assert.deepEqual((await readdir(outputRoot)).sort(), ['schemas', 'src', 'trust', 'verification']);
});
...
test('CLI stages a complete direct-member publication from an exact committed controller fixture', async (t) => {
  ...
  const manifest = JSON.parse(await readFile(
    path.join(outputPath, 'verification', 'controller', 'controller-bundle.manifest.json'),
    'utf8',
  ));
...
const PREPUBLICATION_TESTS = Object.freeze([
  ...
  'verification/adapters/test-cloud/test-cloud-control-store.test.mjs',
  'verification/adapters/test-cloud/test-cloud-lane.test.mjs',
...
function assertPrepublicationGate(workflow) {
  ...
  const bindingIndex = publicationStepIndex(
    ...
    (step) => typeof step.run === 'string'
      && step.run.includes('node verification/controller/src/test-cloud-binding-artifact-verifier.mjs --input'),
    'binding verification',
  );
  const qualificationIndex = publicationStepIndex(
    job.steps,
    (step) => typeof step.run === 'string'
      && step.run.includes('node verification/controller/src/controller-runner-qualification-cli.mjs --input'),
    'runner qualification',
  );
  const stagingIndex = publicationStepIndex(
    job.steps,
    (step) => typeof step.run === 'string'
      && step.run.includes('node verification/controller/src/controller-bundle-publisher.mjs --input'),
    'publication staging',
  );
  ...
  const cases = [
    ...
    ['alternate early publisher step', workflow.replace(
      ...
      '      - name: Alternate preparation\n'
        + '        shell: pwsh\n'
        + '        run: node verification/controller/src/controller-bundle-publisher.mjs --input early.json\n\n'
        + '      - name: Install and run exact prepublication verification\n',
    )],

...
const packagePaths = [
  'verification/controller/workflows/publish-controller-bundle.yml',
...
const HOSTED_WORKFLOW_SEAL = Object.freeze({
  bytes: 7572,
  digest: 'c6c063ed604115e2b3a9fd99ca3b7f2c6d93f91f29c47630dc71ad1dbc53070b',
});
...
function isolatedEnvironmentNames(block) {
  ...
  const end = lines.findIndex((line, index) => (
    index > start
    && line === '            node verification/controller/src/prepare-controller-artifact.mjs \\'
  ));
...
const EXPECTED_PREPARATION_RUN = [
  ...
  '            node verification/controller/src/prepare-controller-artifact.mjs \\',
...
function assertHostedArtifactStaging(workflow) {
  ...
  assert.equal(
    ...
    'node verification/controller/src/test-cloud-controller.mjs --hosted --revision "$REQUESTED_REVISION" --source-run-id "$SOURCE_RUN_ID" --source-run-attempt "$SOURCE_RUN_ATTEMPT" --binding-directory "$BINDING_DIRECTORY"',
  );
...
test('hosted workflow passes the exact verified directory to the controller CLI', async () => {
  const workflow = await readFile(
    'verification/controller/workflows/verify-test-cloud.yml', 'utf8',
  );
...
test('hosted artifact staging rejects admission, ordering, environment, and execution mutations', async () => {
  ...
  const mutations = [
    ...
    workflow.replace(
      'run: node verification/controller/src/test-cloud-controller.mjs',

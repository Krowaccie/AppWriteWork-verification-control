...
} from '../../core/controller-bundle.mjs';
import { createArtifactManifest } from '../../core/artifact-manifest.mjs';
import { canonicalJson, sha256Bytes } from '../../core/canonical-json.mjs';
import { createHostedArtifactHandoff } from '../../core/hosted-artifact-handoff.mjs';
import { createHostedSiteBuildIdentity } from '../../core/hosted-site-build-identity.mjs';
import {
...
import task8ProviderContract from '../../appwrite-runner/provider-contract/test-cloud.provider-contract.v1.json' with { type: 'json' };
import { runTestCloudLane } from '../../../verification/adapters/test-cloud/test-cloud-lane.mjs';
import {
...
} from '../../../verification/adapters/test-cloud/test-cloud-cleanup-protocol.mjs';
import {
  canonicalReadback,
  canonicalReadbackDigest,
  digestJson,
  setupAttestation,
} from '../../core/controller-trust-materials-test-helper.mjs';
import {
  testCloudScenarioInventory,
  testCloudScenarioInventoryDigest,
} from './test-cloud-playwright-facade.mjs';
import { createReleaseEligibleSourceArtifactSet } from '../../../verification/adapters/test-cloud/test-cloud-real-composition-fixture.mjs';
...
const CONTROLLER_WORKFLOW = 'verification/controller/workflows/verify-test-cloud.yml';
const CONTROLLER_MANIFEST = 'verification/controller/controller-bundle.proposal.json';
...
const CONTROLLER_PROTECTED_EVIDENCE_HARNESS_QUERY =
  '?appwritework-controller-protected-evidence-harness=5-final';
...
registerHooks({
  resolve(specifier, context, nextResolve) {
    if (
      ...
      && specifier === '../../../verification/adapters/test-cloud/test-cloud-lane.mjs'
    ) {
      return {
  ...
  load(url, context, nextLoad) {
    ...
    if (url.endsWith(CONTROLLER_PROTECTED_EVIDENCE_HARNESS_QUERY)) {
      return {
        ...loaded,
        source: `${loaded.source}\nexport { buildProtectedVerificationResult as buildProtectedVerificationResultForTest };\n`,
      };
    }
    return loaded;
  },
...
function artifactSetOutput({ sourceTreeDigest = DIGEST, authenticBrowserSite = false } = {}) {
  ...
  const manifest = createArtifactManifest({
    ...
    entries: [...functions, site].map((artifact) => ({
      ...
      sourcePath: artifact.kind === 'site' ? 'apps/web' : sourcePaths.get(artifact.logicalTarget),
...
function controllerArtifactIoFixture() {
  ...
  const schemaPath = 'verification/schemas/controller-bundle.v2.schema.json';
  const manifestPath = 'verification/controller/controller-bundle.manifest.json';
...
function productionFunctionProjections() {
  const records = [
    ...inventory.productFunctions,
    {
      ...
      runtime: 'python-3.12',
      sourcePath: 'verification/appwrite-runner',
    },
  ];
...
function controllerPreflightHandoff() {
  return Object.freeze({
    environmentDigest: digestJson(inventory),
    providerContractDigest: inventory.providerContractDigest,
    identityBindingsDigest: `sha256:${'b'.repeat(64)}`,
    providerSetupReadbackDigest: `sha256:${'c'.repeat(64)}`,
    runnerVariableReadbackDigest: `sha256:${'d'.repeat(64)}`,
    observedLeaseVersion: 6,
    observedLedgerDigest: `sha256:${'e'.repeat(64)}`,
    observedAtEpochSeconds: 1_774_000_000,
  });
}
...
function controllerHarness() {
  ...
  const clients = Object.freeze({
    preflight: stage('preflight', Object.freeze({ handoff: controllerPreflightHandoff() })),
...
async function ordinaryLaneTerminalOutcome() {
  ...
  const source = validatedHostedSnapshot();
  const setup = hostedSetupBinding(controller);
  const generation = hostedConstructionGeneration(controller, setup, source);
  const realLane = containment.status === 'PASS' && createdFacade.status === 'PASS'
    ? await dependencies.createOrdinaryLane({
      artifactSet: source,
      constructionGenerationDigest: digestJson(generation),
      controller,
  ...
  const composed = composeProductionTestCloudLane({
    ...
    operations: Object.freeze({
      preflight: providerTransport(Object.freeze({ handoff: controllerPreflightHandoff() })),
      acquireLease: harness.clients.acquireLease,
...
test('Task 9 vertical: real hosted composition atomically cleans the exact Task 8 fixture state before PASS', async () => {
  ...
  assert.equal(
    ...
    'verification/controller/src/test-cloud-contained-scenario-launcher.mjs',
  );
  assert.deepEqual(
...
test('test-cloud controller module exposes only the closed orchestration and binding entrypoints', async () => {
  ...
  assert.deepEqual(Object.keys(controller).sort(), [
    'TEST_CLOUD_MANAGED_FACTS_CONTEXT',
    'composeProductionTestCloudLane',
    'createProductionHostedDependencies',
    'createTrustedTestCloudAdapterBinding',
    'main',
...
test('hosted controller production composition binds every approved Task 5 runtime contract', async () => {
  const source = await text('verification/controller/src/test-cloud-controller.mjs');
  ...
  assert.match(ordinary, /validConstructionGeneration\(/u);
  assert.ok(
    ordinary.indexOf('validConstructionGeneration(')
      < ordinary.indexOf('admittedCredentials = stage.credentials'),
  );
  assert.ok(
    ordinary.indexOf('validConstructionGeneration(')
      < ordinary.indexOf('createTestCloudClients({'),
  );
  assert.ok(
    ordinary.indexOf('validConstructionGenerationDigest(')
      < ordinary.indexOf('createTestCloudClients({'),
  );
  assert.match(
    ordinary,
    /laneEvidenceMatchesGeneration\([\s\S]*stage\.constructionGenerationDigest/u,
  );
});
...
test('4C-final: controller owns one inert bootstrap call and keeps authority lexical', async () => {
  ...
  const provider = await import(
    '../../../verification/adapters/test-cloud/test-cloud-provider-contract.mjs'
  );
  assert.ok(
...
test('hosted module has one exact trusted lifecycle import allowlist and no candidate import', async () => {
  ...
  assert.deepEqual(imported, [
    ...
    '../../core/process-containment.mjs',
    './test-cloud-source-artifact-reader.mjs',
    '../../core/hosted-site-build-identity.mjs',
    '../../../verification/adapters/test-cloud/test-cloud-environment.mjs',
    '../../../verification/adapters/test-cloud/test-cloud-appwrite-runtime.mjs',
    '../../../verification/adapters/test-cloud/test-cloud-provider-control-runtime.mjs',
    '../../../verification/adapters/test-cloud/test-cloud-identity-bindings.mjs',
    '../../../verification/adapters/test-cloud/test-cloud-setup-check.mjs',
    '../../../verification/adapters/test-cloud/test-cloud-setup-attestation.mjs',
    '../../../verification/adapters/test-cloud/test-cloud-hosted-setup-attestation.mjs',
    '../../../verification/adapters/test-cloud/test-cloud-preflight.mjs',
    '../../../verification/adapters/test-cloud/test-cloud-control-runtime.mjs',
    './test-cloud-fixture-clock.mjs',
    './test-cloud-fixture-intent-producer.mjs',
    './test-cloud-browser-artifact-set.mjs',
    '../../../verification/adapters/test-cloud/test-cloud-deploy.mjs',
    '../../core/evidence.mjs',
    '../../core/evidence-writer.mjs',
  ]);
  assert.match(
    source,
...
test('hosted CLI grammar is exact and the real process fails closed before network access', async () => {
  ...
  const executed = spawnSync(process.execPath, [
    path.join(root, 'verification/controller/src/test-cloud-controller.mjs'),
    ...HOSTED_CLI_ARGS,
  ], {
    cwd: root,
    encoding: 'utf8',
...
function hostedCredentialEnvironment(reads, extra = {}) {
  const values = Object.fromEntries(HOSTED_SECRET_NAMES
...
function hostedSetupBinding(controller = trustedController(), overrides = {}) {
  return Object.freeze({
...
function hostedConstructionGeneration(controller, setup, source) {
  return Object.freeze({
    artifactManifestDigest: source.artifactSet.artifactManifestDigest,
    candidateRevision: source.selection.sourceRevision,
    controllerArtifactId: controller.controllerArtifactId,
    controllerBundleDigest: controller.controllerBundleDigest,
    controllerRevision: controller.controllerBundleSha,
    controllerSourceRepositoryRevision: controller.sourceRepositoryRevision,
    environmentDigest: setup.constructionBinding.environmentDigest,
    hostedSetupReadbackDigest: setup.constructionBinding.hostedSetupReadbackDigest,
    providerContractDigest: setup.constructionBinding.providerContractDigest,
    providerSetupReadbackDigest: setup.constructionBinding.providerSetupReadbackDigest,
    sourceArchiveDigest: source.selection.archiveDigest,
    sourceTreeDigest: source.artifactSet.artifactManifest.sourceTreeDigest,
  });
}

function task5ProtectedEvidenceHarness({
  hostedSetupReadbackDigest = DIGEST,
  providerSetupReadbackDigest = `sha256:${'6'.repeat(64)}`,
} = {}) {
  const controller = trustedController();
  const source = validatedHostedSnapshot();
  const setup = hostedSetupBinding(controller, {
  ...
  const generation = hostedConstructionGeneration(controller, setup, source);
  const constructionGenerationDigest = digestJson(generation);
  return { constructionGenerationDigest, controller, generation, setup, source };
}

function task5BoundLaneEvidence({ controller, generation, source }, cleanupAbsenceDigest) {
  const binding = Object.freeze({
    ...
    controller: Object.freeze({
      ...
      sourceRepositoryRevision: controller.sourceRepositoryRevision,
    }),
    environment: Object.freeze({
      environmentDigest: generation.environmentDigest,
      providerContractDigest: generation.providerContractDigest,
      providerSetupReadbackDigest: generation.providerSetupReadbackDigest,
    }),
    source: Object.freeze({
      archiveDigest: source.selection.archiveDigest,
      sourceRevision: source.selection.sourceRevision,
    }),
    sourceTreeDigest: generation.sourceTreeDigest,
  });
  return Object.freeze({
    binding,
    ...
    siteDeployment: Object.freeze({ artifactTransportDigest: `sha256:${'7'.repeat(64)}` }),
    startedAt: '2026-07-20T10:00:00.000Z',
  });
}

async function writeTask5ProtectedEvidence(harness, cleanupAbsenceDigest) {
  const controllerUrl = pathToFileURL(path.join(
    root,
    'verification',
    'controller',
  ...
  const controllerModule = await import(
    `${controllerUrl}${CONTROLLER_PROTECTED_EVIDENCE_HARNESS_QUERY}`
  );
  assert.equal(
    typeof controllerModule.buildProtectedVerificationResultForTest,
    'function',
  );
  const evidence = await import('../../core/evidence.mjs');
  const created = controllerModule.buildProtectedVerificationResultForTest(evidence, {
    constructionGenerationDigest: harness.constructionGenerationDigest,
    context: Object.freeze({ environmentDigest: harness.generation.environmentDigest }),
    controller: harness.controller,
    generation: harness.generation,
    laneEvidence: task5BoundLaneEvidence(harness, cleanupAbsenceDigest),
    setup: harness.setup,
    source: harness.source,
  });
  assert.notEqual(created, null);
  const { writeVerificationResult } = await import('../../core/evidence-writer.mjs');
  const evidenceRoot = await mkdtemp(path.join(tmpdir(), 'appwritework-c8-task5-evidence-'));
  try {
    await writeFile(path.join(evidenceRoot, '.gitignore'), '.verification/\n', 'utf8');
    const written = await writeVerificationResult({ root: evidenceRoot, result: created });
...
function expectedTask5ProtectedCheckId(harness, cleanupAbsenceDigest) {
  return `test-cloud-protected-evidence-${digestJson({
    cleanupAbsenceDigest,
    constructionGenerationDigest: harness.constructionGenerationDigest,
    environmentIdentityDigest: harness.generation.environmentDigest,
    schemaVersion: 'test-cloud.protected-evidence-binding.v1',
  }).slice('sha256:'.length)}`;
}
...
function hostedDependencies(calls, failAt = null) {
  ...
  return Object.freeze({
    reattestController: stage('controller', controller),
    validateSetupBindings: stage('setup', hostedSetupBinding(controller)),
...
test('Task 5 active hosted ordering blocks every non-Test or incoherent generation before provider construction', async (t) => {
  const { runHostedTestCloudController } = await import('./test-cloud-controller.mjs');
  const exactController = trustedController();
  const productionInventory = structuredClone(inventory);
  productionInventory.environment = {
    endpoint: 'https://api.salmora.net/v1',
    projectId: '69eb4818000afa64a7fa',
    publicOrigin: 'https://salmora.net',
    siteId: '69eb4a020024c520642e',
  };
  const lookalikeInventory = structuredClone(inventory);
  lookalikeInventory.environment.endpoint = 'https://fra.cloud.appwrite.io.evil.invalid/v1';
  const cases = [
    ['production identity', {
      setup: hostedSetupBinding(exactController, { environmentDigest: digestJson(productionInventory) }),
    }],
    ['lookalike identity', {
      setup: hostedSetupBinding(exactController, { environmentDigest: digestJson(lookalikeInventory) }),
    }],
    ['generic production credential', { environment: { APPWRITE_API_KEY: 'generic-forbidden' } }],
    ['recovery credential in ordinary lane', {
      environment: { APPWRITE_TEST_RECOVERY_API_KEY: 'recovery-forbidden' },
    }],
    ['mixed controller generation', {
      setup: hostedSetupBinding(exactController, { controllerRevision: 'f'.repeat(40) }),
    }],
    ['mutable candidate snapshot', { snapshot: structuredClone(validatedHostedSnapshot()) }],
    ['candidate-selected controller', { controller: Object.freeze({ ...exactController }) }],
  ];
    ...
    await t.test(name, async () => {
      ...
      const dependencies = {
        ...
        ...(patch.controller === undefined ? {} : {
          reattestController: async () => pass(patch.controller),
        }),
        ...(patch.setup === undefined ? {} : {
          validateSetupBindings: async () => pass(patch.setup),
        }),
        ...(patch.snapshot === undefined ? {} : {
          validateSourceArtifact: async () => pass(patch.snapshot),
        }),
        async createOrdinaryLane() {
          calls.push('provider-construction');
          return pass(Object.freeze({ lane: true }));
        },
      };
      const outcome = await runHostedTestCloudController({
        request: hostedControllerRequest(),
        environment: hostedCredentialEnvironment(reads, patch.environment),
        dependencies: Object.freeze(dependencies),
      });
      assert.equal(outcome.status, 'BLOCKED', name);
      assert.equal(calls.includes('provider-construction'), false, name);
      assert.equal(calls.includes('lane'), false, name);
    });
  }
});

test('Task 5 setup TOCTOU drift blocks before ordinary lane construction', async () => {
  const { runHostedTestCloudController } = await import('./test-cloud-controller.mjs');
  const controller = trustedController();
  const mutableSetup = {
    constructionBinding: hostedSetupBinding(controller).constructionBinding,
  };
  const calls = [];
  const reads = [];
  const dependencies = {
    ...hostedDependencies(calls),
    async validateSetupBindings() {
      queueMicrotask(async () => {
        await Promise.resolve();
        mutableSetup.constructionBinding = hostedSetupBinding(controller, {
          hostedSetupReadbackDigest: `sha256:${'9'.repeat(64)}`,
        }).constructionBinding;
        calls.push('setup-mutated');
      });
      return pass(mutableSetup);
    },
    async createOrdinaryLane() {
      calls.push('provider-construction');
      return pass(Object.freeze({ lane: true }));
  ...
  assert.equal(outcome.status, 'BLOCKED');
  assert.equal(calls.includes('setup-mutated'), true, JSON.stringify({ calls, outcome }));
  assert.equal(calls.includes('provider-construction'), false);
  assert.equal(calls.includes('lane'), false);
});

test('Task 5 complete generation digest rejects a hosted setup digest mismatch', async () => {
  const { createProductionHostedDependencies } = await import('./test-cloud-controller.mjs');
  const dependencies = createProductionHostedDependencies({
    environment: Object.freeze({}),
    async fetchImpl() {
    ...
    async runContainedProcessImpl(options) {
      return Object.freeze({
        ...
        exitCode: 0,
        signal: null,
        stdout: options.args[0] === '--eval'
          ? 'APPWRITEWORK_WINDOWS_JOB_OBJECT_QUALIFIED'
          : '',
        stderr: '',
      });
    },
  ...
  const containment = await dependencies.qualifyContainment();
  const facade = await dependencies.createPlaywrightFacade();
  assert.equal(containment.status, 'PASS');
  assert.equal(facade.status, 'PASS');
  const controller = trustedController();
  const source = validatedHostedSnapshot();
  const setup = hostedSetupBinding(controller, {
    hostedSetupReadbackDigest: `sha256:${'8'.repeat(64)}`,
  });
  const generation = hostedConstructionGeneration(controller, setup, source);
  const otherHostedDigestGeneration = Object.freeze({
    ...generation,
    hostedSetupReadbackDigest: DIGEST,
  });

  const outcome = await dependencies.createOrdinaryLane({
    artifactSet: source,
    constructionGenerationDigest: digestJson(otherHostedDigestGeneration),
    controller,
    credentials: Object.freeze(Object.fromEntries(
      HOSTED_SECRET_NAMES
        .filter((name) => name !== 'SOURCE_ARTIFACT_READER_PRIVATE_KEY')
...
test('Task 5 persisted evidence binds a hosted-only construction generation change', async () => {
  const cleanupAbsenceDigest = `sha256:${'d'.repeat(64)}`;
  const baseline = await task5ProtectedEvidenceHarness({
    hostedSetupReadbackDigest: `sha256:${'8'.repeat(64)}`,
  });
  const changed = await task5ProtectedEvidenceHarness({
    hostedSetupReadbackDigest: `sha256:${'9'.repeat(64)}`,
  });

  const baselineWrite = await writeTask5ProtectedEvidence(baseline, cleanupAbsenceDigest);
  const changedWrite = await writeTask5ProtectedEvidence(changed, cleanupAbsenceDigest);

  assert.equal(
    baselineWrite.persisted.environmentIdentityDigest,
    baseline.generation.environmentDigest,
  );
  assert.equal(
    changedWrite.persisted.environmentIdentityDigest,
    changed.generation.environmentDigest,
  );
  assert.equal(
    baselineWrite.persisted.environmentIdentityDigest,
    changedWrite.persisted.environmentIdentityDigest,
  );
  assert.notEqual(baselineWrite.outcome.value.path, changedWrite.outcome.value.path);
  assert.notEqual(
    baselineWrite.outcome.value.evidenceDigest,
    changedWrite.outcome.value.evidenceDigest,
  );
  assert.notEqual(baselineWrite.persisted.resultId, changedWrite.persisted.resultId);
  assert.notEqual(
    baselineWrite.persisted.selectedChecks.find((checkId) => (
      checkId.startsWith('test-cloud-protected-evidence-')
    )),
    changedWrite.persisted.selectedChecks.find((checkId) => (
      checkId.startsWith('test-cloud-protected-evidence-')
    )),
  );
  for (const [harness, write] of [
    [baseline, baselineWrite],
    [changed, changedWrite],
  ]) {
    const protectedCheckId = expectedTask5ProtectedCheckId(harness, cleanupAbsenceDigest);
    assert.match(protectedCheckId, /^test-cloud-protected-evidence-[0-9a-f]{64}$/u);
    assert.deepEqual(write.persisted.selectedChecks, [
      'hosted-test-cloud',
      protectedCheckId,
    ].sort());
    assert.deepEqual(
      write.persisted.checks.map(({ checkId, diagnostics, status }) => ({
        checkId,
        diagnostics,
        status,
      })),
      [
        { checkId: 'hosted-test-cloud', diagnostics: [], status: 'PASS' },
        { checkId: protectedCheckId, diagnostics: [], status: 'PASS' },
      ].sort((left, right) => left.checkId.localeCompare(right.checkId)),
    );
    const persistedText = canonicalJson(write.persisted);
    assert.equal(persistedText.includes(harness.constructionGenerationDigest), false);
    assert.equal(persistedText.includes(harness.generation.hostedSetupReadbackDigest), false);
    assert.equal(persistedText.includes(harness.generation.providerSetupReadbackDigest), false);
    assert.equal(persistedText.includes(cleanupAbsenceDigest), false);
  }
});

test('Task 5 persisted evidence binds a cleanup-absence-only change', async () => {
  const harness = await task5ProtectedEvidenceHarness({
    hostedSetupReadbackDigest: `sha256:${'8'.repeat(64)}`,
  });
  const baselineCleanupAbsenceDigest = `sha256:${'d'.repeat(64)}`;
  const changedCleanupAbsenceDigest = `sha256:${'e'.repeat(64)}`;

  const baselineWrite = await writeTask5ProtectedEvidence(
    harness,
    baselineCleanupAbsenceDigest,
  );
  const changedWrite = await writeTask5ProtectedEvidence(
    harness,
    changedCleanupAbsenceDigest,
  );

  assert.equal(
    baselineWrite.persisted.environmentIdentityDigest,
    harness.generation.environmentDigest,
  );
  assert.equal(
    changedWrite.persisted.environmentIdentityDigest,
    harness.generation.environmentDigest,
  ...
  for (const [cleanupAbsenceDigest, write] of [
    [baselineCleanupAbsenceDigest, baselineWrite],
    [changedCleanupAbsenceDigest, changedWrite],
  ]) {
    const protectedCheckId = expectedTask5ProtectedCheckId(harness, cleanupAbsenceDigest);
    assert.match(protectedCheckId, /^test-cloud-protected-evidence-[0-9a-f]{64}$/u);
    assert.deepEqual(write.persisted.selectedChecks, [
...
test('production hosted setup derives policy only from hosted bytes and confirms all eight bindings', async () => {
  ...
  assert.deepEqual(outcome.value.constructionBinding, hostedSetupBinding(trustedController(), {
    hostedSetupReadbackDigest: environment.TEST_CLOUD_HOSTED_SETUP_READBACK_DIGEST,
    providerSetupReadbackDigest: environment.TEST_CLOUD_SETUP_READBACK_DIGEST,
  }).constructionBinding);
});
...
test('inert test-cloud entrypoints cannot mint a vars-only trusted controller context', async () => {
  const controllerSource = await text(
    'verification/controller/src/test-cloud-controller.mjs',
  );
  const preparationSource = await text(
    'verification/controller/src/prepare-controller-artifact.mjs',
  );
  const workflow = await text(CONTROLLER_WORKFLOW);
  ...
  assert.match(
    workflow,
    /REQUIRED_CONTROLLER_ENTRYPOINT:\s*verification\/controller\/workflows\/verify-test-cloud\.yml/u,
  );
...
test('controller bundle inventory is additive but ineligible until an exact committed tuple exists', async () => {
  ...
  for (const required of [
    'verification/controller/src/appwrite-test-browser-policy.mjs',
    'verification/controller/src/appwrite-test-live-readback.mjs',
    'verification/controller/src/appwrite-test-setup-bindings.mjs',
    'verification/controller/src/collect-appwrite-test-readback.mjs',
    'verification/controller/src/test-cloud-binding-artifact-verifier.mjs',
    'verification/controller/src/test-cloud-controller.mjs',
    'verification/controller/src/release-production.mjs',
    'verification/controller/src/run-production-readonly-playwright.mjs',
    'verification/controller/workflows/collect-appwrite-test-readback.yml',
    'verification/controller/workflows/verify-test-cloud.yml',
    'verification/controller/workflows/release-production.yml',
    'verification/controller/workflows/production-readonly.yml',
  ]) {

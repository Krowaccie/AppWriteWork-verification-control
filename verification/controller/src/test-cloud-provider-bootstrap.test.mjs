...
import { createTestEnvironmentContext } from '../../../verification/adapters/test-cloud/test-cloud-environment.mjs';

const ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '../../..');
const inventory = JSON.parse(await readFile(
  path.join(ROOT, 'verification/fixtures/environments/test-cloud.inventory.v1.json'),
  'utf8',
));
const providerContractPath =
  'verification/appwrite-runner/provider-contract/test-cloud.provider-contract.v1.json';
...
test('provider bootstrap qualification remains valid for the authentic test-cloud context', async () => {
  const provider = await import('../../../verification/adapters/test-cloud/test-cloud-provider-contract.mjs');

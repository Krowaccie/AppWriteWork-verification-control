...
import * as hostedArtifactHandoff from '../../core/hosted-artifact-handoff.mjs';
...
function validBundleEntries({ extra = false } = {}) {
  const artifacts = [
    { kind: 'site', logicalTarget: 'production-site', sourcePath: 'apps/web', relativePath: 'site/site.zip' },
    ...Array.from({ length: 35 }, (_, index) => {
      const logicalTarget = 'function-' + String(index + 1).padStart(2, '0');
      return { kind: 'function', logicalTarget, sourcePath: 'services/appwrite-functions/' + logicalTarget, relativePath: 'functions/' + logicalTarget + '.tar.gz' };
    }),
    { kind: 'function', logicalTarget: 'verification-runner-py', sourcePath: 'verification/appwrite-runner', relativePath: 'functions/verification-runner-py.tar.gz' },
  ].map((record, index) => {

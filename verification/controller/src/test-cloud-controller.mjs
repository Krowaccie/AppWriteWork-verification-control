...
import { validateArtifactManifest } from '../../core/artifact-manifest.mjs';
import { canonicalJson } from '../../core/canonical-json.mjs';
import { isTrustedControllerContext } from '../../core/controller-bundle.mjs';
import { validateHostedSiteBuildIdentity } from '../../core/hosted-site-build-identity.mjs';
import configuredInventory from '../../fixtures/environments/test-cloud.inventory.v1.json' with { type: 'json' };
import {
  runTestCloudLane,
  validateTestCloudArtifactSet,
} from '../../../verification/adapters/test-cloud/test-cloud-lane.mjs';
import { createTestCloudAdapter } from '../../../verification/adapters/test-cloud/test-cloud-adapter.mjs';
import { runTrustedTestCloudCleanup } from './test-cloud-cleanup-driver.mjs';
import { QUALIFIED_CLEANUP_PROTOCOL } from '../../../verification/adapters/test-cloud/test-cloud-cleanup-protocol.mjs';
...
const loadProviderRuntime = () =>
  import('../../../verification/adapters/test-cloud/test-cloud-provider-contract.mjs');
...
const loadProcessContainment = () =>
  import('../../core/process-containment.mjs');
const loadSourceArtifactReader = () =>
  import('./test-cloud-source-artifact-reader.mjs');
const loadHostedSiteIdentity = () =>
  import('../../core/hosted-site-build-identity.mjs');
const loadTestEnvironment = () =>
  import('../../../verification/adapters/test-cloud/test-cloud-environment.mjs');
const loadTestCloudAppwrite = () =>
  import('../../../verification/adapters/test-cloud/test-cloud-appwrite-runtime.mjs');
const loadProviderControlStore = () =>
  import('../../../verification/adapters/test-cloud/test-cloud-provider-control-runtime.mjs');
const loadIdentityBindings = () =>
  import('../../../verification/adapters/test-cloud/test-cloud-identity-bindings.mjs');
const loadSetupCheck = () =>
  import('../../../verification/adapters/test-cloud/test-cloud-setup-check.mjs');
const loadSetupAttestation = () =>
  import('../../../verification/adapters/test-cloud/test-cloud-setup-attestation.mjs');
const loadHostedSetupAttestation = () =>
  import('../../../verification/adapters/test-cloud/test-cloud-hosted-setup-attestation.mjs');
const loadTestCloudPreflight = () =>
  import('../../../verification/adapters/test-cloud/test-cloud-preflight.mjs');
const loadTestCloudControlStore = () =>
  import('../../../verification/adapters/test-cloud/test-cloud-control-runtime.mjs');
...
const loadTestCloudDeploy = () =>
  import('../../../verification/adapters/test-cloud/test-cloud-deploy.mjs');
const loadEvidence = () => import('../../core/evidence.mjs');
const loadEvidenceWriter = () =>
  import('../../core/evidence-writer.mjs');
...
const SETUP_CONSTRUCTION_BINDING_KEYS = Object.freeze([
  'controllerArtifactId',
  'controllerBundleDigest',
  'controllerRevision',
  'controllerSourceRepositoryRevision',
  'environmentDigest',
  'hostedSetupReadbackDigest',
  'providerContractDigest',
  'providerSetupReadbackDigest',
].sort());
const CONSTRUCTION_GENERATION_KEYS = Object.freeze([
  'artifactManifestDigest',
  'candidateRevision',
  'controllerArtifactId',
  'controllerBundleDigest',
...
const PROTECTED_VERIFICATION_RESULT_ARGUMENT_KEYS = Object.freeze([
  'constructionGenerationDigest',
  'context',
  'controller',
  'generation',
  'laneEvidence',
...
const FORBIDDEN_ORDINARY_CREDENTIAL_NAMES = Object.freeze([
  ...configuredInventory.productionDenylist.credentialVariableNames,
  configuredInventory.credentialVariables.recovery.variableName,
]);
...
const TEST_CLOUD_MANAGED_FACT_ROWS = Object.freeze([
  Object.freeze([
    'managed-project-provider-settings',
    'test-cloud.managed-project-provider-settings.v1',
  ]),
  Object.freeze([
    'managed-function-site-domains',
    'test-cloud.managed-function-site-domains.v1',
  ]),
  Object.freeze(['public-tls-dns', 'test-cloud.public-tls-dns.v1']),
  Object.freeze([
    'managed-scaling-regions-backups-cdn',
    'test-cloud.managed-scaling-regions-backups-cdn.v1',
  ]),
  Object.freeze([
    'external-sandbox-behavior',
    'test-cloud.external-sandbox-behavior.v1',
  ]),
  Object.freeze([
    'appwrite-cloud-self-host-policy-differences',
    'test-cloud.appwrite-cloud-self-host-policy-differences.v1',
  ]),
]);

export const TEST_CLOUD_MANAGED_FACTS_CONTEXT = deepFreeze({
  factsSchema: {
    type: 'object',
    additionalProperties: false,
    required: ['evidenceDigest', 'managedCloudFacts'],
    properties: {
      evidenceDigest: { type: 'string', pattern: '^sha256:[0-9a-f]{64}$' },
      managedCloudFacts: {
        type: 'array',
        maxItems: TEST_CLOUD_MANAGED_FACT_ROWS.length,
        items: {
          oneOf: TEST_CLOUD_MANAGED_FACT_ROWS.map(([category, factId]) => ({
            type: 'object',
            additionalProperties: false,
            required: [
              'category',
            ...
            properties: {
              ...
              factId: { const: factId },
              status: { enum: ['PASS', 'FAIL', 'BLOCKED', 'INFRA_ERROR'] },
              publicOrigin: { const: configuredInventory.environment.publicOrigin },
              evidenceDigest: { type: 'string', pattern: '^sha256:[0-9a-f]{64}$' },
            },
          })),
        },
...
export function createTrustedTestCloudAdapterBinding(args) {
  if (!exactDataObject(args, ['controller', 'dependencies'])) throw dependencyError();
  const created = createTestCloudAdapter(args);
  if (created.status !== 'PASS') return created;
  return result('PASS', {
    adapter: created.value,
    normalizationContext: TEST_CLOUD_MANAGED_FACTS_CONTEXT,
  });
}
...
function deeplyFrozenOrdinaryData(value, seen = new WeakSet()) {
  try {
    if (value === null || typeof value !== 'object') return true;
    if (ArrayBuffer.isView(value)) return !isProxy(value);
    if (isProxy(value) || seen.has(value) || !Object.isFrozen(value)) return false;
    ...
    for (const key of Reflect.ownKeys(value)) {
      ...
      if (!deeplyFrozenOrdinaryData(descriptor.value, seen)) return false;
    }
    return true;
  } catch {
    return false;
...
function validSetupConstructionBinding(value, controller) {
  const binding = ownDataValue(value, 'constructionBinding');
  return exactFrozenOrdinaryDataRecord(binding, SETUP_CONSTRUCTION_BINDING_KEYS)
    && binding.controllerArtifactId === controller.controllerArtifactId
    && binding.controllerBundleDigest === controller.controllerBundleDigest
    && binding.controllerRevision === controller.controllerBundleSha
    && binding.controllerSourceRepositoryRevision === controller.sourceRepositoryRevision
    && binding.environmentDigest === TEST_CLOUD_ENVIRONMENT_DIGEST
    && binding.providerContractDigest === configuredInventory.providerContractDigest
    && DIGEST.test(binding.hostedSetupReadbackDigest)
    && DIGEST.test(binding.providerSetupReadbackDigest);
}

function buildConstructionGeneration(controller, setup, source) {
  const binding = setup.constructionBinding;
  return deepFreeze({
    ...
    hostedSetupReadbackDigest: binding.hostedSetupReadbackDigest,
    providerContractDigest: binding.providerContractDigest,
    providerSetupReadbackDigest: binding.providerSetupReadbackDigest,
    sourceArchiveDigest: source.selection.archiveDigest,
    sourceTreeDigest: source.artifactSet.artifactManifest.sourceTreeDigest,
  });
}

function validConstructionGeneration(value, controller, setup, source) {
  if (!exactFrozenOrdinaryDataRecord(value, CONSTRUCTION_GENERATION_KEYS)) return false;
  const expected = buildConstructionGeneration(controller, setup, source);
  return canonicalJson(value) === canonicalJson(expected);
}

function constructionGenerationDigest(generation) {
  return digestBytes(new TextEncoder().encode(canonicalJson(generation)));
}

function validConstructionGenerationDigest(value, generation) {
  return typeof value === 'string'
    && DIGEST.test(value)
    && value === constructionGenerationDigest(generation);
}

function hasForbiddenOrdinaryCredential(environment) {
  try {
    return FORBIDDEN_ORDINARY_CREDENTIAL_NAMES.some((name) => (
      Object.getOwnPropertyDescriptor(environment, name) !== undefined
    ));
  } catch {
    return true;
  }
...
function laneEvidenceMatchesGeneration(
  laneEvidence,
  generation,
  expectedConstructionGenerationDigest,
) {
  try {
    const binding = laneEvidence.binding;
    return deeplyFrozenOrdinaryData(laneEvidence)
      && validConstructionGenerationDigest(
        expectedConstructionGenerationDigest,
        generation,
      )
      && laneEvidence.bindingDigest === digestBytes(
        new TextEncoder().encode(canonicalJson(binding)),
      )
      && binding.source.sourceRevision === generation.candidateRevision
      && binding.source.archiveDigest === generation.sourceArchiveDigest
      && binding.sourceTreeDigest === generation.sourceTreeDigest
      && binding.artifactManifestDigest === generation.artifactManifestDigest
      && binding.controller.sourceRepositoryRevision
        === generation.controllerSourceRepositoryRevision
      && binding.controller.controllerBundleSha === generation.controllerRevision
      && binding.controller.controllerBundleDigest === generation.controllerBundleDigest
      && binding.controller.controllerArtifactId === generation.controllerArtifactId
      && binding.environment.environmentDigest === generation.environmentDigest
      && binding.environment.providerContractDigest === generation.providerContractDigest
      && binding.environment.providerSetupReadbackDigest
        === generation.providerSetupReadbackDigest
      && DIGEST.test(binding.cleanupAbsenceDigest)
      && laneEvidence.cleanupAbsenceProven === true;
  } catch {
    return false;
  }
}

function buildProtectedVerificationResult(evidence, args) {
  try {
    if (
      !exactDataObject(args, PROTECTED_VERIFICATION_RESULT_ARGUMENT_KEYS)
      || !validConstructionGeneration(
        args.generation,
        args.controller,
        args.setup,
        args.source,
      ...
      || !validConstructionGenerationDigest(
        args.constructionGenerationDigest,
        args.generation,
      )
      || !laneEvidenceMatchesGeneration(
        args.laneEvidence,
        args.generation,
        args.constructionGenerationDigest,
      )
    ) return null;
    const environmentIdentityDigest = ownDataValue(args.context, 'environmentDigest');
    const cleanupAbsenceDigest = ownDataValue(
      ownDataValue(args.laneEvidence, 'binding'),
      'cleanupAbsenceDigest',
    );
    const siteDeployment = ownDataValue(args.laneEvidence, 'siteDeployment');
    const artifactTransportDigest = ownDataValue(siteDeployment, 'artifactTransportDigest');
    const startedAt = ownDataValue(args.laneEvidence, 'startedAt');
    const completedAt = ownDataValue(args.laneEvidence, 'completedAt');
    const started = Date.parse(startedAt);
    const completed = Date.parse(completedAt);
    if (
      environmentIdentityDigest !== args.generation.environmentDigest
      || !DIGEST.test(environmentIdentityDigest)
      || !DIGEST.test(cleanupAbsenceDigest)
      || !DIGEST.test(artifactTransportDigest)
      || !Number.isFinite(started)
      || !Number.isFinite(completed)
      || completed < started
    ) return null;
    const protectedEnvelopeDigest = digestBytes(new TextEncoder().encode(canonicalJson({
      cleanupAbsenceDigest,
      constructionGenerationDigest: args.constructionGenerationDigest,
      environmentIdentityDigest,
      schemaVersion: 'test-cloud.protected-evidence-binding.v1',
    })));
    const protectedCheckId = `test-cloud-protected-evidence-${
      protectedEnvelopeDigest.slice('sha256:'.length)
    }`;
    return evidence.createVerificationResult({
      bootstrapQualification: {
        bundleDigest: args.controller.controllerBundleDigest,
        status: 'PASS',
        verifierRevision: args.controller.controllerBundleSha,
      },
      candidateArtifactDigest: args.source.selection.archiveDigest,
      candidateRevision: args.source.selection.sourceRevision,
      candidateSourceTreeDigest: args.source.artifactSet.artifactManifest.sourceTreeDigest,
      checks: [
        {
          attempts: 1,
      ...
      environmentIdentityDigest,
      lane: 'test-cloud',
      manifestDigest: args.source.artifactSet.artifactManifestDigest,
      observedDeployment: {
        artifactDigest: artifactTransportDigest,
        readbackSource: 'public-build-identity',
        releaseRecordDigest: null,
        releaseRecordId: null,
        revision: args.source.selection.sourceRevision,
      },
      selectedChecks: ['hosted-test-cloud', protectedCheckId],
      startedAt,
      verifierRevision: args.controller.controllerBundleSha,
    });
  } catch {
  ...
  const dependencies = {
    ...
    async validateSetupBindings(stage) {
      ...
      if (
        ...
        || checked.value.sourceRepositoryRevision
          !== stage.controller.sourceRepositoryRevision
        || hostedReadback.value.controller.bundle.artifactId
          !== stage.controller.controllerArtifactId
        || providerReadback.value.environmentDigest !== TEST_CLOUD_ENVIRONMENT_DIGEST
      ) return result('BLOCKED', null, 'TEST_CLOUD_SETUP_INCOMPLETE');
      ...
      return result('PASS', deepFreeze({
        ...
        attestationDigest: providerAttestation.digest,
        constructionBinding: {
          controllerArtifactId: stage.controller.controllerArtifactId,
          controllerBundleDigest: stage.controller.controllerBundleDigest,
          controllerRevision: stage.controller.controllerBundleSha,
          controllerSourceRepositoryRevision: stage.controller.sourceRepositoryRevision,
          environmentDigest: TEST_CLOUD_ENVIRONMENT_DIGEST,
          hostedSetupReadbackDigest: hostedReadback.digest,
          providerContractDigest: configuredInventory.providerContractDigest,
          providerSetupReadbackDigest: providerReadback.digest,
        },
        executionObservationQualification: observation.value,
    ...
    async createOrdinaryLane(stage) {
      if (!validConstructionGeneration(
        stage.generation,
        stage.controller,
        stage.setup,
        stage.artifactSet,
      ) || !validConstructionGenerationDigest(
        stage.constructionGenerationDigest,
        stage.generation,
      )) return result('BLOCKED', null, 'TEST_CLOUD_PREFLIGHT_BLOCKED');
      ...
      const laneEvidenceWriter = Object.freeze({
        async write(laneEvidence) {
          if (!laneEvidenceMatchesGeneration(
            laneEvidence,
            stage.generation,
            stage.constructionGenerationDigest,
          )) {
          ...
          const verificationResult = buildProtectedVerificationResult(evidence, {
            constructionGenerationDigest: stage.constructionGenerationDigest,
            context,
...
export async function runHostedTestCloudController(args) {
  ...
  if (
    ...
    || !isTrustedControllerContext(controller.value)
  ...
  if (
    setup.status !== 'PASS'
    || !validSetupConstructionBinding(setup.value, controller.value)
    || !deeplyFrozenOrdinaryData(setup.value)
    || hasForbiddenOrdinaryCredential(args.environment)
  ) return result('BLOCKED', null, 'TEST_CLOUD_SETUP_INCOMPLETE');
  const setupSnapshot = setup.value;

  const runtime = await hostedStage(
    args.dependencies.bootstrapRuntime,
    { ...shared, controller: controller.value, setup: setupSnapshot },
  ...
  if (
    ...
    || !deeplyFrozenOrdinaryData(artifact.value)
  ) return result('BLOCKED', null, 'SOURCE_ARTIFACT_INVALID');

  const generation = buildConstructionGeneration(
    controller.value,
    setupSnapshot,
    artifact.value,
  );
  ...
  const generatedConstructionDigest = constructionGenerationDigest(generation);
  ...
  const lane = await hostedStage(
    args.dependencies.createOrdinaryLane,
    {
      ...
      constructionGenerationDigest: generatedConstructionDigest,
      controller: controller.value,

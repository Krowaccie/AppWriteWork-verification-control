...
const workflows = ['verification/controller/workflows/collect-appwrite-test-readback.yml'];
const COLLECTOR_WORKFLOW_SEAL = Object.freeze({
  bytes: 7673,
  digest: '772153cc8cd5a88ac6dbc5feded3598692edfe2558c0cf2284e6c2739821f223',
});
...
const EXPECTED_COLLECTOR_COMMAND = 'node verification/controller/src/collect-appwrite-test-readback.mjs --input $env:COLLECTOR_INPUT --output $env:COLLECTOR_OUTPUT';
...
test('protected readback workflow contract rejects permission and secret-command mutations', async (t) => {
  ...
  const cases = [
    ...
    [
      ...
      text.replace(
        'node verification/controller/src/collect-appwrite-test-readback.mjs --input $env:COLLECTOR_INPUT --output $env:COLLECTOR_OUTPUT',
        'node verification/controller/src/collect-appwrite-test-readback.mjs --input $env:COLLECTOR_INPUT --output $env:COLLECTOR_OUTPUT --operator-api-key $env:APPWRITE_TEST_OPERATOR_API_KEY',
      ),
    ],
  ];

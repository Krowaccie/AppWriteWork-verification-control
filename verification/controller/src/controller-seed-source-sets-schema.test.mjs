...
} from '../../core/controller-source-set-contract.mjs';

const TEST_READER = 'verification/controller/src/test-cloud-source-artifact-reader.mjs';
const TEST_READER_TEST = 'verification/controller/src/test-cloud-source-artifact-reader.test.mjs';
...
const TEST_PROVIDER_CORPUS = 'verification/fixtures/test-cloud/test-cloud-provider-contract.v1.corpus.json';
const TEST_SETUP_CORPUS = 'verification/fixtures/test-cloud/test-cloud-setup-readback.v1.corpus.json';
...
const TRUST_MATERIALS_TEST_HELPER = 'verification/core/controller-trust-materials-test-helper.mjs';
const HOSTED_ARTIFACT_HANDOFF = 'verification/core/hosted-artifact-handoff.mjs';
const TEST_CONTROL_HELPER = 'verification/adapters/test-cloud/test-cloud-control-test-helper.mjs';
const TEST_REAL_COMPOSITION_FIXTURE = 'verification/adapters/test-cloud/test-cloud-real-composition-fixture.mjs';
const VALIDATION_WORKFLOW = 'verification/controller/workflows/controller-validation.yml';
const CONTROLLER_VALIDATION_TESTS = Object.freeze([
  'verification/controller/src/appwrite-test-browser-policy.test.mjs',
  'verification/controller/src/appwrite-test-live-readback.test.mjs',
  'verification/controller/src/appwrite-test-setup-bindings.test.mjs',
  'verification/controller/src/collect-appwrite-test-readback-workflow.test.mjs',
  'verification/controller/src/collect-appwrite-test-readback.test.mjs',
  'verification/controller/src/controller-bundle-publisher.test.mjs',
  'verification/controller/src/controller-seed-source-sets-schema.test.mjs',
  'verification/controller/src/prepare-controller-artifact.test.mjs',
  'verification/controller/src/production-exact-sha-git-adapter.test.mjs',
  'verification/controller/src/recover-appwrite-test-workflow.test.mjs',
  'verification/controller/src/source-artifact-reader.test.mjs',
  'verification/controller/src/test-cloud-binding-artifact-verifier.test.mjs',
  'verification/controller/src/test-cloud-binding-workflows.test.mjs',
  'verification/controller/src/test-cloud-browser-artifact-set.test.mjs',
  'verification/controller/src/test-cloud-browser-route-adapter.test.mjs',
  'verification/controller/src/test-cloud-cleanup-driver.test.mjs',
  'verification/controller/src/test-cloud-contained-browser-scenario-adapter.test.mjs',
  'verification/controller/src/test-cloud-controller-binding-directory.test.mjs',
  'verification/controller/src/test-cloud-controller-source-diagnostics.test.mjs',
  'verification/controller/src/test-cloud-controller.test.mjs',
  'verification/controller/src/test-cloud-fixture-clock.test.mjs',
  'verification/controller/src/test-cloud-fixture-intent-producer.test.mjs',
  'verification/controller/src/test-cloud-provider-bootstrap.test.mjs',
  'verification/controller/src/test-cloud-recovery-controller.test.mjs',
  TEST_READER_TEST,
]);
...
async function validationDependencyGraph(sourceSets) {
  ...
  const overlaySourceRoot = 'verification/controller/controller-repository-seed';
  ...
  const packageJson = JSON.parse(await readFile('verification/controller/package.json', 'utf8'));
...
test('controller seed schema and descriptor select only the final validation closure', async () => {
  const [schema, sourceSets] = await Promise.all([
    readFile('verification/schemas/controller-seed-source-sets.v1.schema.json', 'utf8')
      .then(JSON.parse),
    readFile('verification/controller/controller-seed-source-sets.v1.json', 'utf8')
      .then(JSON.parse),
  ]);
  ...
  assert.equal(requiredFiles.includes('verification/controller/workflows/recover-appwrite-test.yml'), true);
...
test('Tasks 10-11 descriptor closes the approved validation inventory and literal validation relocation', async () => {
  const sourceSets = JSON.parse(await readFile(
    'verification/controller/controller-seed-source-sets.v1.json',
    'utf8',
  ));
  ...
  inferredFromSuffix.sets[3].files = [
    'verification/controller/src/arbitrary.test.mjs',
  ];
...
test('static validation dependency discovery is scope-aware and fails closed on root ambiguity', () => {
  const shadowed = staticReferences('verification/controller/src/example.test.mjs', `
  ...
  const directRelative = staticReferences('verification/controller/src/example.test.mjs', `
    import { readFile } from 'node:fs/promises';
    await readFile('./root-relative.json');
  `);
  assert.deepEqual(directRelative.local, ['root-relative.json']);

  const wrapperCollision = staticReferences('verification/controller/src/example.test.mjs', `
    import { readFile } from 'node:fs/promises';
  ...
  assert.throws(
    () => staticReferences('verification/controller/src/example.test.mjs', `
      import { readFile } from 'node:fs/promises';
      import path from 'node:path';

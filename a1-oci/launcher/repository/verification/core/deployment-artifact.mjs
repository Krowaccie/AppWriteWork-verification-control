...
const RUNNER_SOURCE_PATH = 'verification/appwrite-runner';
const PRODUCT_LOGICAL_IDS = Object.freeze([
  'api-keys-py',
  'api-router-py',
  'billing-cron-py',
  'billing-py',
...
const PRODUCT_LOGICAL_ID_SET = new Set(PRODUCT_LOGICAL_IDS);
const PRODUCT_RESERVED_ARCHIVE_ROOTS = new Set([
  'appwritework_backend',
  'packages',
  'shared_py',
  'src',
]);
...
function readInput(value) {
  ...
  const isRunner = unit.logicalId === RUNNER_ID;
  if (isRunner) {
    if (unit.sourcePath !== RUNNER_SOURCE_PATH) {
      return { error: 'ARTIFACT_SCHEMA_INVALID' };
    }
  } else if (
    !PRODUCT_LOGICAL_ID_SET.has(unit.logicalId)
    || unit.sourcePath !== `services/appwrite-functions/${unit.logicalId}`
  ) {
    return { error: 'ARTIFACT_SCHEMA_INVALID' };
  }
  return {
    ...
    sourceType: isRunner ? 'runner' : 'product',
...
export async function buildFunctionArtifact(args) {
  ...
  try {
    ...
    let entries;
    if (input.sourceType === 'runner') {
      entries = [...functionEntries];
      if (entries.some(({ archivePath }) => archivePath === RUNNER_IDENTITY_PATH)) {
        throw new ArtifactPathError();
      }
      ...
      if (functionEntries.some(({ archivePath }) => (
        PRODUCT_RESERVED_ARCHIVE_ROOTS.has(archivePath.split('/')[0].toLowerCase())
      ))) throw new ArtifactPathError();
      ...
      const backendEntries = await collectDirectory(
        backendRoot,
        'appwritework_backend',
        exportReal,
      );
      if (backendEntries.length === 0) throw new ArtifactBuildError();
      entries = [...functionEntries, ...backendEntries];
    }

...
function validFunctionEntry(candidate, expectedLogicalId) {
  ...
  const expectedSourcePath = expectedLogicalId === 'verification-runner-py'
    ? 'verification/appwrite-runner'
    : `services/appwrite-functions/${expectedLogicalId}`;
  return entry !== null
...
function createCommandSpec(state, commandId) {
  const workspace = state.workspace.workspace;
  const webRoot = reflectApply(pathPosixJoin, PathPosix, [workspace.exportRoot, 'apps', 'web']);
  let executable;
  let args;
  let cwd;
  let network;
  ...
  switch (commandId) {
    ...
    case 'bundle-catalog':
      ...
      args = ['tools/build/bundle-catalog.mjs'];
      cwd = workspace.exportRoot;
      network = 'deny';
      break;
    case 'typecheck':
